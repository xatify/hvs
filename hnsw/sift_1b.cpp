	...
	int d)
{
	...
	double rand_;
	for (int i = 0; i < L; i++)
	{ // generate L intial centers randomly
		rand_ = double(i) / L;
		index = int((n - 1) * rand_);
		if (index < 0 || index >= n)
		{
	...
	int efsearch)
{
	...
	if (chdir(index_dir) != 0) {
		std::cout << "index directory not found" << std::endl;
		exit(1);
	}
	...
	for (int i = 0; i < size_n; i++)
	{
		...
		if (i > 0)
		{
			input.seekg((4 + 4 * vecdim) * (interval - 1), std::ios::cur);
		}
		...
		for (int ii = 0; ii < length[i - 1]; ii++)
		{
			for (int jj = ii + 1; jj < length[i - 1]; jj++)
			{
				...
				for (int i1 = 0; i1 < L; i1++)
				{
					...
					{
						if (con_count[i1][j1] == 0 || count1[jj][j1] == 0)
							continue;
...
#pragma omp parallel for
	...
	{ // not deal with level 0
		...
		for (int j = 0; j < vecsize; j++)
		{
			...
			{
				if (fflag[i - 1][j] == true)
				{ // the representative exists in the base layer
					element2.id = j;
				...
				{
					init_obj[i][j] = j;
				} // connect to representative in the base layer
			}
		}
	}
